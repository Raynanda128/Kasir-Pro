...
export const LoginView: React.FC<LoginViewProps> = ({ onLoginSuccess, initialMode }) => {
  ...
  useEffect(() => {
    ...
    if (window.location.hash.includes('invite=') || window.location.search.includes('invite=')) {
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-slate-800/90 backdrop-blur-xl p-8 rounded-3xl border border-slate-700/80 shadow-2xl">
          ...
          {hasInviteToken && (
            <div className="mb-4 p-3.5 bg-emerald-950/80 border border-emerald-700/80 text-emerald-200 rounded-2xl text-xs space-y-1">
              <div className="font-bold flex items-center gap-1.5 text-emerald-300">
                <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
                <span>Undangan Bergabung Ditemukan!</span>
              </div>
              <p className="text-[11px] leading-relaxed text-emerald-200/90">
                Silakan <strong>Login</strong> (jika sudah punya akun) atau <strong>Daftar Akun Baru</strong> menggunakan email staf yang diundang untuk langsung terhubung ke toko.
              </p>
            </div>
          )}
          ...
          {mode === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 mb-1">Nama Cabang / Toko Pertama</label>
                <div className="relative">
              ...
              <div className="p-3 bg-emerald-950/40 border border-emerald-800/60 rounded-xl text-xs text-emerald-300 flex items-center gap-2">
                ...
                <span>Pendaftaran akun ini otomatis membuat <strong>Bisnis & Cabang Utama Baru</strong> (Hak Akses Pemilik / Owner).</span>
              </div>
