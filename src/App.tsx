...
function AppContent() {
  ...
  const handleChangeRole = (role: UserRole) => {
    ...
    // Restrict access if tab not permitted
    if (role === 'Kasir' && ['produk', 'stok', 'laporan', 'pengaturan'].includes(activeTab)) {
      setActiveTab('kasir');
    } else if (role === 'Admin' && ['laporan', 'pengaturan'].includes(activeTab)) {
      setActiveTab('kasir');
    }
  };
  ...
  const handleResetData = async () => {
    if (window.confirm('Apakah Anda yakin ingin mereset seluruh database KasirPro ke data sampel awal?')) {
      try {
        await apiResetData();
        showToast('Database KasirPro telah di-reset ke data sampel!', 'success');
      } catch (err: any) {
