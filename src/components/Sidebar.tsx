...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const bizRole = currentUser.businessRole || (role === 'Owner' ? 'owner' : role === 'Admin' ? 'admin' : 'cashier');
  ...
  const allowedItems = navItems.filter(item => item.roles.includes(bizRole));
    ...
    <>
      ...
      `}>
        ...
        <div 
          ...
          className="p-3.5 mx-4 my-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 flex items-center gap-3 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group relative"
          title="Klik untuk ubah foto profil, nama & kata sandi"
        >
          <div className="relative">
            <img 
              src={currentUser.avatar || DEFAULT_BOTAK_AVATAR} 
              alt={currentUser.name}
              className="w-10 h-10 rounded-full object-cover shrink-0 border-2 border-emerald-500/30 group-hover:border-emerald-500 transition-colors"
            />
            <div className="absolute -bottom-1 -right-1 p-1 bg-emerald-600 text-white rounded-full text-[9px] shadow-xs opacity-80 group-hover:opacity-100 transition-opacity">
